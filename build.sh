...
BITLITAS_DIR=bitlitas
BITLITASD_EXEC=bitlitasd
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/bitlitas-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    BITLITASD_EXEC=bitlitasd.exe
fi
...
$QMAKE ../bitlitas-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy bitlitasd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$BITLITAS_DIR/bin/$BITLITASD_EXEC $BIN_PATH
fi
