...
# Base environnement
        ...
        cd bitlitas/utils/build_scripts
        docker build -f android32.Dockerfile -t bitlitas-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t bitlitas-gui-android .
        docker create -it --name bitlitas-gui-android bitlitas-gui-android bash

# Get the apk

        docker cp bitlitas-gui-android:/opt/android/bitlitas-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/bitlitas-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
