; Bitlitas Helium Hydra GUI Wallet Installer for Windows
; Copyright (c) 2018, Bitlitas
; See LICENSE

[Setup]
AppName=Bitlitas GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Bitlitas GUI Wallet
DefaultGroupName=Bitlitas GUI Wallet
UninstallDisplayIcon={app}\bitlitas-wallet-gui.exe
...
; As of version 0.11.1.0, the Bitlitas .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Bitlitas.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Bitlitas GUI wallet
Source: "bin\bitlitas-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bitlitas GUI wallet log file
...
Source: "bitlitas-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Bitlitas CLI wallet
Source: "bin\bitlitas-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bitlitas wallet RPC interface implementation
Source: "bin\bitlitas-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bitlitas daemon
Source: "bin\bitlitasd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Bitlitas daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "bitlitas-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Bitlitas blockchain utilities
Source: "bin\bitlitas-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\bitlitas-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\bitlitas-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\bitlitas-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitbitlitas');
  s := 'The default folder to store the Bitlitas blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitbitlitas.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "bitlitas-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Bitlitas daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\bitlitasd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\bitlitas-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Bitlitas GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\bitlitas-wallet-gui.exe"
...
Name: "{group}\Utilities\Bitlitas Daemon"; Filename: "{app}\bitlitasd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\bitlitas-wallet-cli.exe"; WorkingDir: "{userdocs}\Bitlitas\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Bitlitas\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\bitlitas-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\bitlitas-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM bitlitasd.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\bitlitas-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\bitlitas-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\bitlitas-project\bitlitas-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\bitlitas-project\bitlitas-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
