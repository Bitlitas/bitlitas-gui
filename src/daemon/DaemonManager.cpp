...
{
    // prepare command line arguments and pass to bitlitasd
    ...
    qDebug() << "starting bitlitasd " + m_bitlitasd;
    ...
    // Start bitlitasd
    bool started = m_daemon->startDetached(m_bitlitasd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM bitlitasd.exe");
#else
                QProcess::execute("pkill bitlitasd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./bitlitasd status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_bitlitasd, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to bitlitasd
#ifdef Q_OS_WIN
    m_bitlitasd = QApplication::applicationDirPath() + "/bitlitasd.exe";
#elif defined(Q_OS_UNIX)
    m_bitlitasd = QApplication::applicationDirPath() + "/bitlitasd";
#endif

    if (m_bitlitasd.length() == 0) {
