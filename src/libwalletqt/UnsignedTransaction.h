...
public:
    enum Status {
        Status_Ok       = Bitlitas::UnsignedTransaction::Status_Ok,
        Status_Error    = Bitlitas::UnsignedTransaction::Status_Error,
        Status_Critical    = Bitlitas::UnsignedTransaction::Status_Critical
    };
    Q_ENUM(Status)
...
private:
    explicit UnsignedTransaction(Bitlitas::UnsignedTransaction * pt, Bitlitas::Wallet *walletImpl, QObject *parent = 0);
    ~UnsignedTransaction();
private:
    friend class Wallet;
    Bitlitas::UnsignedTransaction * m_pimpl;
    QString m_fileName;
