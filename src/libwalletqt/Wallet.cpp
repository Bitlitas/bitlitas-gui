...
class WalletListenerImpl : public  Bitlitas::WalletListener
{
public:
...
{
    QFuture<Bitlitas::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Bitlitas::Wallet::connected);
    QFutureWatcher<Bitlitas::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Bitlitas::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Bitlitas::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Bitlitas::Wallet::ConnectionStatus> future = connectionWatcher->future();
...
{
    Bitlitas::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount, mixin_count,
                static_cast<Bitlitas::PendingTransaction::Priority>(priority));
...
{
    Bitlitas::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Bitlitas::optional<uint64_t>(), mixin_count,
                static_cast<Bitlitas::PendingTransaction::Priority>(priority));
...
{
    Bitlitas::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Bitlitas::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Bitlitas::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitbitlitas.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/bitlitas-wallet-gui.log";
}

Wallet::Wallet(Bitlitas::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_history;
    m_history = NULL;
    //Bitlitas::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
