...
#include "wallet/api/wallet2_api.h" // we need to have an access to the Bitlitas::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Bitlitas::Wallet::Status_Ok,
        Status_Error    = Bitlitas::Wallet::Status_Error,
        Status_Critical = Bitlitas::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Bitlitas::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Bitlitas::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Bitlitas::Wallet::ConnectionStatus_WrongVersion
    };
...
private:
    ...
    Wallet(Bitlitas::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
    friend class WalletManager;
    friend class WalletListenerImpl;
    //! libwallet's
    Bitlitas::Wallet * m_walletImpl;
